...
# Настройка времени хранения файлов (например, 1 час)
FILE_EXPIRATION_TIME = timedelta(hours=1)
...
def remove_old_files():
    """Удаляет файлы старше FILE_EXPIRATION_TIME."""
    now = datetime.now()
    for filename in os.listdir(TEMP_FOLDER):
        ...
        if os.path.isfile(file_path):
            ...
            if now - file_creation_time > FILE_EXPIRATION_TIME:
                try:
                    os.remove(file_path)
                    logging.info(f"🗑 Файл {filename} удален за истечением срока хранения")
                except Exception as e:
                    logging.error(f"❌ Ошибка при удалении {filename}: {e}")


# Запуск фонового планировщика для удаления старых файлов
scheduler = BackgroundScheduler()
scheduler.add_job(remove_old_files, "interval", minutes=30)
scheduler.start()
